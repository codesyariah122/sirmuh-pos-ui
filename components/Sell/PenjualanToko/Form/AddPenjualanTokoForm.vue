  ...
  <div>
    <div class="flex justify-start space-x-0">
      ...
      <div class="shrink-0 w-60 text-black">
        ...
        <span class="text-blueGray-800">Preparing data kas</span>
      </div>
      <div v-else>
        <Select2
...
<span class="font-semibold text-blueGray-800">Preparing data kas</span>
</div>
<div v-else>
  <div
...
<div>
  <div class="flex justify-start space-x-0 py-6">
    ...
    <div v-else class="shrink-0 w-60 text-black">
      ...
      <span class="text-blueGray-800">Preparing data pelanggan</span>
    </div>
    <div v-else>
      <Select2
...
<script>
  ...
  export default {
      ...
      async getPelangganLists() {
        ...
        const getAllPages = async () => {
          ...
          while (currentPage <= totalPages) {
            const data = await getData({
              api_url: `${this.api_url}/list-pelanggan-normal?page=${currentPage}`,
              token: this.token.token,
              api_key: this.api_token,
            });
    ...
    simpanPenjualan(draft) {
      ...
      if(this.input.pembayaran === 'custom') {
        if(this.input.bayar === null) {
          formData.append("bayar", 0);
        } else {
          formData.append("bayar", this.input.bayar);
        }
        formData.append("diterima", this.input.diterima);
      } else {
        formData.append("bayar", this.showKembali ? this.input.bayar : this.total);
        formData.append("diterima", this.showKembali ? this.input.diterima : this.total);
      }
