...
<div class="flex-auto px-4 lg:px-10 py-10 pt-0">
  <form @submit.prevent="updateSuppplier">
...
<div class="w-full lg:w-6/12 px-4">
  ...
  class="border-0 px-3 py-3 placeholder-blueGray-300 text-blueGray-600 bg-white rounded text-sm shadow focus:outline-none focus:ring w-full ease-linear transition-all duration-150"
  v-model="detail.no_npwp"
  />
</div>
<div
v-if="validations.no_npwp"
class="flex p-4 mb-4 text-sm text-red-800 rounded-lg bg-red-50 dark:bg-gray-800 dark:text-red-400"
role="alert"
>
<i class="fa-solid fa-circle-info"></i>
<div class="px-2">
  {{ validations.no_npwp[0] }}
</div>
</div>
</div>
</div>
...
<div class="flex flex-wrap">
  ...
  <span v-else><i class="fa-solid fa-plus"></i> Update Supplier</span>
</button>
...
<script>
  export default {
    ...
    methods: {
      ...
      inputNamaSupplier(e) {
        this.detail.nama = e.target.value
        this.detail.kode = this.createInitial(this.detail.nama);
      },
      ...
      updateSuppplier() {
        ...
        const prepareData = {
          ...
          npwp: this.input.no_npwp ? this.input.no_npwp : this.detail.no_npwp,
        };

        const endPoint = `/data-supplier/${this.slug}`;
        const config = {
          headers: {
