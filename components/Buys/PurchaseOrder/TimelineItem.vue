<template>
  <div>
    <li v-for="(groupedTimeline, idx) in groupedTimelines" :key="groupedTimeline.kode_barang" class="mb-10 ms-4">
      <div
      ...
      >
      <span
      class="bg-blue-100 text-blue-800 text-lg font-medium me-2 px-2.5 py-0.5 rounded border border-blue-400"
      >
  ...
  <p class="text-base font-normal text-gray-500">
    QTY : {{ $roundup(groupedTimeline.reduce((acc, timeline) => acc + parseInt(timeline.qty), 0)) }} {{ items[idx].satuan }}
  </p>
  <p class="mb-4 text-base font-normal text-gray-500">
    Harga Satuan : {{ $format(groupedTimeline[0].harga_satuan) }}
  </p>
  <p class="mb-4 text-base font-normal text-gray-500">
    Supplier : <span class="bg-green-100 text-green-800 text-lg font-medium me-2 px-2.5 py-0.5 rounded border border-green-400">
