    ...
    <li class="w-full py-2">
      <div class="grid grid-cols-3 gap-0">
        ...
        <div>
          ...
          @input="inputBiayaBongkar"
          />
        </div>
      </div>
    </li>

    <li v-if="parseFloat(detail?.saldo_hutang) !== 0" class="py-6">
      <div class="grid grid-cols-3 gap-0">
        <div>
          <label class="font-bold">Saldo Hutang</label>
        </div>
        <div>
          <input
          disabled
      ...
      <div class="flex justify-start mb-4 py-2">
        <div>
          <div class="flex items-center p-4 mb-4 text-sm text-blue-800 rounded-lg bg-blue-50 dark:bg-gray-800 dark:text-blue-400" role="alert">
            ...
            <div>
              <span class="font-medium">Jumlah Saldo Hutang!</span> masih ada saldo hutang sebesar : {{detail.saldo_hutang}}.
            </div>
          </div>
        </div>
      </div>
...
data() {
  return {
    ...
    detailKasBiaya: {},
    showDetailKas: null,
    showDetailKasBiaya: null,
    loadingKas: null,
    loadingKasBiaya: null,
    showKembali:
    this.detail &&
...
methods: {
  inputKasBiaya() {
    this.showBiaya = false;
    this.showBiayaKas = false;
    this.selectedKasBiaya = null;
    this.input.biayabongkar = 0;
  },
  checkItemMultiInput() {
  ...
  .then(({ data }) => {
    if(data.success) {
      ...
      if (data.data.lunas === "True") {
        ...
        if(data.data.bayar < data.data.diterima) {
          this.generateTerbilang(parseFloat(data.data.diterima))
          this.masukHutang = true;
          this.modeBayar = true;
          this.showBayar = false;
          this.showDp = false;
          this.hutangAfter = true;
          this.kembali = `Hutang : ${this.$format(Math.abs(data.data.diterima - parseFloat(data.data.jumlah)))}`;
          ...
          this.modeBayar = false;
          this.showDp = true;
          const sisaDp = data.sisa_dp ? data.sisa_dp : parseFloat(data.data.bayar) - data.data.diterima
...
changeGantiHarga(e, id, barang) {
  ...
  if(e.key === 'Escape') {
    ...
    this.input.harga = this.initialHarga;
    barang.harga_beli = this.initialHarga;
    this.editingItemId = null
  } else if(e.key === 'Enter') {
    this.showGantiHarga = false
    this.input.harga = newHarga;
...
changeBayar(e) {
  ...
  const numBayar = parseFloat(this.detail.jumlah) + bayar;
  ...
  this.input.bayar = parseFloat(bayar);
  this.input.diterima = parseFloat(bayar);
...
inputBiayaBongkar(e) {
  this.input.biayabongkar = Number(e.target.value)
},
...
changeKasBiaya(newValues) {
  if (newValues && newValues.id !== undefined) {
    const kasId = Number(newValues.id);
    if (!isNaN(kasId)) {
      this.showBiaya = true;
      this.selectedKasBiaya = kasId;
      this.getKasBiayaDetail(kasId);
      this.input.kas_biaya = kasId;
    }
  }
},
...
async getKasBiayaDetail(id) {
  this.loadingKasBiaya = true;
  const data = await getData({
    api_url: `${this.api_url}/data-kas/${id}`,
    token: this.token.token,
    api_key: this.api_token,
  ...
  setTimeout(() => {
    this.showDetailKasBiaya = true;
    this.detailKasBiaya = result;
    this.loadingKasBiaya = false;
  }, 500);
},
...
recalculateJumlahRupiah(isi = 0, diskon = 0) {
  ...
  this.total += ppnAmount;
    ...
    updatePembelian(draft) {
      ...
      const calculateBayar = parseFloat(this.detail.jumlah) + parseFloat(this.input.bayar);
      const prepareItem = {
