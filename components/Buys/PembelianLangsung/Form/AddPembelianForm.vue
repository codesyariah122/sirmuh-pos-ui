    ...
    <form @submit.prevent="simpanPembelian(false)">
      ...
      >
        <div class="shrink w-[80vw]">
          ...
          >
          <div v-if="loadingSaldo" class="grid justify-items-center">
              <div>
                <div role="status">
...
export default {
  ...
  methods: {
    ...
    updateQty(id, draft) {
      let selectedBarangQty
      if (draft) {
        selectedBarangQty = this.listDraftCarts
          .map((item) => item)
        ...
        if (selectedBarangQty) {
          ...
          this.updateStokBarang(selectedBarangQty.id_barang);
        ...
        selectedBarangQty = this.barangCarts
          .map((item) => item)
          .find((item) => item.id === id);
    ...
    updateStokBarang(id) {
      const endPoint = `/update-stok-barang/${id}`;
      ...
      if(this.barangCarts.length > 0) {
        prepareData = this.barangCarts.map(item => item)
        .find(item => item.id === id)
      } else {
        prepareData = this.listDraftCarts.map(item => item)
        .find(item => item.id_barang === id)
      }
    ...
    simpanPembelian(draft) {
      ...
      let prepareBarang;
      if(this.listDraftCarts.length > 0) {
        prepareBarang = this.listDraftCarts.map((item) => ({
          id: item.id_barang,
          qty: item.qty,
        }));        
      } else {
        prepareBarang = this.barangCarts.map((item) => ({
          id: item.id,
          qty: item.qty,
        }));
      }
