...
<div>
  <div class="flex justify-start space-x-0 py-6">
    ...
    <div class="shrink-0 w-full">
      ...
      >
      <small>
        Contoh : Kesalahan pembelian edit quantity
      </small>
      <textarea
      id="keterangan"
      rows="4"
  ...
  <div
  v-if="error && validation?.keterangan"
  class="mt-2 p-2 mb-4 text-sm text-red-800 rounded-lg bg-red-50 dark:bg-gray-800 dark:text-red-400"
  role="alert"
  >
  <span class="font-medium">Danger alert!</span>
...
<td v-else class="px-6 py-4">
  <div class="flex justify-between whitespace-nowrap space-x-2">
    <div>
...
recalculateJumlahRupiah(isi = 0, diskon = 0) {
  ...
  this.total += ppnAmount;
    ...
    updateStokBarang() {
      const endPoint = `/edit-stok-data-barang`;
      const config = {
        headers: {
          Authorization: `Bearer ${this.token.token}`,
        },
      ...
      .then(({ data }) => {
        ...
        if (data.success) {
          if (data.data.lunas === "True") {
            this.showKembali = true;
            if (parseFloat(data.data.bayar) < parseFloat(data.data.jumlah)) {
              this.masukHutang = true;
              this.modeBayar = true;
