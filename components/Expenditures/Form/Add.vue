...
<div class="flex-auto px-4 lg:px-10 py-10 pt-0">
  <form @submit.prevent="addNewPengeluaran">
...
<div class="w-full lg:w-6/12 px-4">
  ...
  <Select2
  v-model="selectedBiaya"
  :settings="{
    allowClear: true,
    dropdownCss: { top: 'auto', bottom: 'auto' },
  }"
  :options="[{ id: null, text: 'Pilih Kode Biaya' }, ...biayas]"
  @change="changeKodeBiaya($event)"
  @select="changeKodeBiaya($event)"
  placeholder="Pilih Kode Biaya"
  />
</div>
<div
v-if="validations.kd_biaya"
class="flex p-4 mb-4 text-sm text-red-800 rounded-lg bg-red-50 dark:bg-gray-800 dark:text-red-400"
role="alert"
>
<i class="fa-solid fa-circle-info"></i>
<div class="px-2">
  {{ validations.kd_biaya[0] }}
</div>
</div>
</div>
    ...
    methods: {
      async generateReferenceCode() {
        ...
        if (refCodeStorage && refCodeStorage?.ref_code !== null) {
          ...
          const data = await getData({
            api_url: `${this.api_url}/generate-reference-code/pengeluaran`,
      ...
      changeKodeBiaya(newValue) {
        ...
        if (biaya !== undefined) {
          ...
          this.input.keterangan = `Menambahkan pengeluaran untuk ${newValue.text}`
        }
      },

      transformDataBiaya(rawData) {
        return rawData
      ...
      getBiayaLists() {
        ...
        getAllPages()
        .then((data) => {
          this.biayas = this.transformDataBiaya(data);
        })
        .finally(() => {
          this.loadingKodeBiaya = false;
        })
      ...
      addNewPengeluaran() {
        ...
        const endPoint = `/data-pengeluaran`;
        const config = {
          headers: {
