  ...
  <div class="flex flex-wrap mt-12">
    <div class="w-full">
      <div class="flex justify-start space-x-6">
        ...
        <div v-if="loadingJenisPemasukan">
          <div role="status">
      ...
      <div class="flex justify-start space-x-6 mt-6">
        <div class="col-span-full">
          <div class="flex items-center">
            <input
              :checked="$nuxt.viewAllPemasukan"
              id="checked-checkbox"
              type="checkbox"
              value=""
              @change="handleView"
              v-model="$nuxt.viewAllPemasukan"
              class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600"
            />
            <label
              for="checked-checkbox"
              class="ms-2 text-sm font-medium text-gray-900 dark:text-gray-300"
              >{{$nuxt.viewAllPemasukan ? 'Tampilkan bulan ini saja' : 'Menampilkan seluruh data'}}</label
            >
          </div>
        </div>
      </div>
...
export default {
  ...
  methods: {
    ...
    clearSelectedData() {
      this.selectedBiaya = "";
      this.clearKey += 1;
      this.$nuxt.viewAllPemasukan = true;
      this.$emit("filter-data", {
        keywords: "",
        biaya: this.selectedBiaya,
        date: "",
        view_all: true,
      });
    },
    ...
    getDataBiayaLists() {
      ...
      getAllPages()
        .then((data) => {
          this.biayas = this.transformKodeBiaya(data);
        })
        .finally(() => {
