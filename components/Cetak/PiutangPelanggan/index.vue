...
export default {
  ...
  methods: {
    changeType(type) {
      this.type = type;
      this.showModalPerusahaan = !this.showModalPerusahaan;
    },

    changePerusahaan(newValue) {
      const perusahaanId = newValue.id;
      if (perusahaanId !== undefined) {
        const printUrl = `${this.server_url}/transaksi/terima-piutang/cetak-nota/${this.type}/${this.kode}/${perusahaanId}`;
