<template>
  <div class="flex flex-wrap mt-4">
    <div class="w-full mb-12 px-4">
      <cards-card-table color="dark" title="DATA MEMBER" types="data-member" queryType="DATA_MEMBER"
        queryMiddle="data-member" :parentRoute="stringRoute" :typeRoute="typeRoute" :headers="headers" :columns="items"
...
export default {
  ...
  methods: {
    ...
    deleteBarang(id) {
      this.loading = true;
      this.options = "delete-data-member";
      deleteData({
        api_url: `${this.api_url}/data-member/${id}`,
        token: this.token.token,
        api_key: process.env.NUXT_ENV_APP_TOKEN,
      })
        .then((data) => {
  ...
  watch: {
    notifs() {
      if (this.$_.size(this.$nuxt.notifs) > 0) {
        ...
        if (this.$nuxt.notifs[0].routes === "data-member") {
