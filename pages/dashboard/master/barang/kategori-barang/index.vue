  ...
  <div class="flex flex-wrap mt-4">
    <div :class="`${$nuxt.showSidebar ? 'w-full mb-12  px-6' : 'max-w-full'}`">
      <cards-card-table
        ...
        types="kategori-barang"
        queryType="KATEGORI_BARANG"
        queryMiddle="barang"
        :headers="headers"
        :columns="items"
        :loading="loading"
 ...
 */
import { KATEGORI_DATA_TABLE } from "~/utils/table-kategori-barang";
...
export default {
  ...
  methods: {
    ...
    getDataBiaya(page = 1, param = {}, loading) {
      ...
      getData({
        api_url: `${this.api_url}/data-kategori-barang?page=${page}${
          param.nama
            ? "&keywords=" + param.nama
            : ""
        }`,
