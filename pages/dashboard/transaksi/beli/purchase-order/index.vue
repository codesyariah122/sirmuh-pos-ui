  ...
  export default {
    ...
    methods: {
      ...
      getPurchaseOrder(page = 1, param = {}, loading) {
        ...
        const endPoint = `${this.api_url}/data-purchase-order?${param.keyword ? 'keywords='+param.keyword+'&' : ''}page=${page}&view_all=${this.$nuxt.viewAllPembelianPO}${param.date ? "&date_transaction=" + param.date :""}${param.supplier ? '&supplier='+param.supplier : supplier ? '&supplier='+supplier : ''}`
          ...
          if (data?.success) {
            data?.data?.map((cell) => {
              const prepareCell = {
                ...
                jumlah: parseFloat(cell?.jumlah) + parseFloat(cell?.biayabongkar),
                biayabongkar: cell?.biayabongkar,
                bayar: cell?.bayar,
