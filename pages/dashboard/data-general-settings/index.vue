<template>
  <div class="flex flex-wrap mt-4">
    <div class="w-full mb-12 px-4">
      <cards-card-table color="dark" title="DATA GENERAL SETTINGS" types="data-general-settings"
        queryType="DATA_GENERAL_SETTINGS" queryMiddle="data-general-settings" :headers="headers" :columns="items"
...
export default {
  ...
  methods: {
    handleFilterBarang(param, types) {
      if (types === "data-general-settings") {
    ...
    deleteBarang(id) {
      this.loading = true;
      this.options = "delete-general-settings";
      deleteData({
        api_url: `${this.api_url}/data-general-settings/${id}`,
        token: this.token.token,
        api_key: process.env.NUXT_ENV_APP_TOKEN,
      })
        .then((data) => {
