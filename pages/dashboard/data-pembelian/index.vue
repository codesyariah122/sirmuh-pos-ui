<template>
  <div class="flex flex-wrap mt-4">
    <div class="w-full mb-12 px-4">
      <cards-card-table color="dark" title="DATA PEMBELIAN" types="data-pembelian" queryType="DATA_PEMBELIAN"
        queryMiddle="data-pembelian" :headers="headers" :columns="items" :loading="loading" :success="success"
...
export default {
  ...
  methods: {
    ...
    deleteBarang(id) {
      this.loading = true;
      this.options = "delete-pembelian";
      deleteData({
        api_url: `${this.api_url}/data-pembelian/${id}`,
        token: this.token.token,
        api_key: process.env.NUXT_ENV_APP_TOKEN,
      })
        .then((data) => {
