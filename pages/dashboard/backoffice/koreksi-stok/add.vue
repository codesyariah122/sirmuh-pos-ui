<template>
  <div class="flex flex-wrap">
    ...
    <cards-card-settings
    color="dark"
    pageType="koreksiStok"
    link="backoffice"
    title="Koreksi Stok Baru"
    methodType="add"
    :type="type"
    pageData="/backoffice/koreksi-stok"
    />
  </div>
</div>
</template>
    ...
    watch: {
      notifs() {
        if (this.$nuxt.notifs && this.$_.size(this.$nuxt.notifs) > 0) {
          if (this.$nuxt.notifs.find(notif => notif.routes === "koreksi-stok") || this.$nuxt.notifs.find(notif => notif.routes === "data-barang")) {
            this.storedFormData();
          }
        }
      },
