<template>
  <div class="flex flex-wrap mt-4">
    <div :class="`${$nuxt.showSidebar ? 'w-full mb-12' : '-ml-10 max-w-full'}`">
      ...
      types="laporan-hutang-jatuh-tempo"
      queryType="DATA_LAPORAN_HUTANG"
      queryMiddle="laporan-hutang-jatuh-tempo"
 ...
 */
  ...
  export default {
    name: "laporan-hutang-jatuh-tempo",
    layout: "admin",
    ...
    methods: {
      handleFilterBarang(param, types) {
        if (types === "laporan-hutang-jatuh-tempo") {
      ...
      deleteBarang(id) {
        this.loading = true;
        this.options = "delete-laporan-hutang-jatuh-tempo";
        deleteData({
          api_url: `${this.api_url}/laporan-hutang-jatuh-tempo/${id}`,
          token: this.token.token,
          api_key: process.env.NUXT_ENV_APP_TOKEN,
        })
        .then((data) => {
