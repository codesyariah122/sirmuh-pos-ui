<template>
  <div class="flex flex-wrap mt-4">
    <div class="w-full mb-12 px-4">
      <cards-card-table color="dark" title="LAPORAN PEMBAYARAN HUTANG SUPPLIER" types="laporan-pembayaran-hutang-supplier"
        queryType="LAPORAN_PEMBAYARAN_HUTANG_SUPPLIER" queryMiddle="laporan-pembayaran-hutang-supplier" :headers="headers"
...
export default {
  name: "laporan-pembayaran-hutang-supplier",
  layout: "admin",
  ...
  methods: {
    handleFilterBarang(param, types) {
      if (types === "laporan-pembayaran-hutang-supplier") {
    ...
    deleteBarang(id) {
      this.loading = true;
      this.options = "delete-laporan-pembayaran-hutang-supplier";
      deleteData({
        api_url: `${this.api_url}/laporan-pembayaran-hutang-supplier/${id}`,
        token: this.token.token,
        api_key: process.env.NUXT_ENV_APP_TOKEN,
      })
        .then((data) => {
