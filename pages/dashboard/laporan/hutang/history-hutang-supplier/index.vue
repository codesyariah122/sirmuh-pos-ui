<template>
  <div class="flex flex-wrap mt-4">
    <div class="w-full mb-12 px-4">
      <cards-card-table color="dark" title="HISTORY HUTANG SUPPLIER" types="history-hutang-supplier"
        queryType="HISTORY_HUTANG_SUPPLIER" queryMiddle="history-hutang-supplier" :headers="headers" :columns="items"
...
export default {
  ...
  methods: {
    handleFilterBarang(param, types) {
      if (types === "history-hutang-supplier") {
    ...
    deleteBarang(id) {
      this.loading = true;
      this.options = "delete-history-hutang-supplier";
      deleteData({
        api_url: `${this.api_url}/history-hutang-supplier/${id}`,
        token: this.token.token,
        api_key: process.env.NUXT_ENV_APP_TOKEN,
      })
        .then((data) => {
