<template>
  <div class="flex flex-wrap mt-4">
    <div class="w-full mb-12 px-4">
      <cards-card-table color="dark" title="LAPORAN TUKAR POINT" types="laporan-tukar-point"
        queryType="LAPORAN_TUKAR_POINT" queryMiddle="laporan-tukar-point" :headers="headers" :columns="items"
...
export default {
  ...
  methods: {
    ...
    deleteBarang(id) {
      this.loading = true;
      this.options = "delete-laporan-tukar-point";
      deleteData({
        api_url: `${this.api_url}/laporan-tukar-point/${id}`,
        token: this.token.token,
        api_key: process.env.NUXT_ENV_APP_TOKEN,
      })
        .then((data) => {
