...
export default {
  name: "history-point-member-dari-supplier",
  layout: "admin",
  ...
  methods: {
    handleFilterBarang(param, types) {
      if (types === "history-point-member-dari-supplier") {
    ...
    deleteBarang(id) {
      this.loading = true;
      this.options = "delete-history-point-member-dari-supplier";
      deleteData({
        api_url: `${this.api_url}/history-point-member-dari-supplier/${id}`,
        token: this.token.token,
        api_key: process.env.NUXT_ENV_APP_TOKEN,
      })
        .then((data) => {
