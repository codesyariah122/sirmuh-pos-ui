<template>
  <div class="flex flex-wrap mt-4">
    <div class="w-full mb-12 px-4">
      <cards-card-table color="dark" title="LAPORAN HISTORY POINT MEMBER" types="laporan-history-point-member"
        queryType="LAPORAN_HISTORY_POINT_MEMBER" queryMiddle="laporan-history-point-member" :headers="headers"
...
export default {
  name: "laporan-history-point-member",
  layout: "admin",
  ...
  methods: {
    handleFilterBarang(param, types) {
      if (types === "laporan-history-point-member") {
    ...
    deleteBarang(id) {
      this.loading = true;
      this.options = "delete-laporan-history-point-member";
      deleteData({
        api_url: `${this.api_url}/laporan-history-point-member/${id}`,
        token: this.token.token,
        api_key: process.env.NUXT_ENV_APP_TOKEN,
      })
        .then((data) => {
