<template>
  <div class="flex flex-wrap mt-4">
    <div class="w-full mb-12 px-4">
      <cards-card-table color="dark" title="LAPORAN POINT MEMBER" types="laporan-point-member"
        queryType="LAPORAN_POINT_MEMBER" queryMiddle="laporan-point-member" :headers="headers" :columns="items"
...
export default {
  ...
  methods: {
    ...
    deleteBarang(id) {
      this.loading = true;
      this.options = "delete-laporan-point-member";
      deleteData({
        api_url: `${this.api_url}/laporan-point-member/${id}`,
        token: this.token.token,
        api_key: process.env.NUXT_ENV_APP_TOKEN,
      })
        .then((data) => {
