<template>
  <div class="flex flex-wrap mt-4">
    <div :class="`${$nuxt.showSidebar ? 'w-full mb-12 px-12' : 'max-w-full'}`">
      <cards-card-table
        color="light"
        title="DATA LAPORAN PENJUALAN PERIODE"
        types="laporan-penjualan-periode"
        queryType="DATA_LAPORAN_PENJUALAN_PERIODE"
        queryMiddle="laporan-penjualan-periode"
        :headers="headers"
        :columns="items"
        :loading="loading"
        :success="success"
...
import { PENJUALAN_LAPORAN_DATA_TABLE } from "~/utils/table-data-laporan-penjualan";

export default {
  name: "laporan-penjualan-periode",
  layout: "admin",
  ...
  data() {
    return {
      ...
      success: null,
      message_success: "",
      headers: [...PENJUALAN_LAPORAN_DATA_TABLE],
  ...
  methods: {
    handleFilterLaporanPembelianPeriode(param, types) {
      if (types === "laporan-penjualan-periode") {
    ...
    changePerusahaan(newValue) {
      ...
      if (perusahaanId !== undefined) {
        const printUrl = `${this.server_url}/laporan/penjualan/laporan-penjualan-periode/${perusahaanId}/${this.start_date}/${this.end_date}`;
    ...
    getDataLaporanPembelianPeriode(page = 1, param = {}) {
      ...
      this.$nuxt.globalLoadingMessage =
        "Proses menyiapkan data laporan penjualan periode ...";

      getData({
        api_url: `${this.api_url}/laporan-penjualan?page=${page}${
          param.keyword
    ...
    deletePelanggan(id) {
      ...
      deleteData({
        api_url: `${this.api_url}/laporan-penjualan/${id}`,
        token: this.token.token,
        api_key: process.env.NUXT_ENV_APP_TOKEN,
      })
        .then((data) => {
