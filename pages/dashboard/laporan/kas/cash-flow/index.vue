<template>
  <div class="flex flex-wrap mt-4">
    <div class="w-full mb-12 px-4">
      <cards-card-table color="dark" title="CASH FLOW" types="cash-flow" queryType="CASH_FLOW" queryMiddle="cash-flow"
...
export default {
  ...
  methods: {
    ...
    deleteBarang(id) {
      this.loading = true;
      this.options = "delete-cash-flow";
      deleteData({
        api_url: `${this.api_url}/cash-flow/${id}`,
        token: this.token.token,
        api_key: process.env.NUXT_ENV_APP_TOKEN,
      })
        .then((data) => {
