<template>
  <div class="flex flex-wrap mt-4">
    <div class="w-full mb-12 px-4">
      <cards-card-table color="dark" title="DATA LAPORAN PENGELUARAN" types="data-laporan-pengeluaran"
        queryType="DATA_LAPORAN_PENGELUARAN" queryMiddle="data-laporan-pengeluaran" :headers="headers" :columns="items"
...
export default {
  ...
  methods: {
    handleFilterBarang(param, types) {
      if (types === "data-laporan-pengeluaran") {
    ...
    deleteBarang(id) {
      this.loading = true;
      this.options = "delete-data-laporan-pengeluaran";
      deleteData({
        api_url: `${this.api_url}/data-laporan-pengeluaran/${id}`,
        token: this.token.token,
        api_key: process.env.NUXT_ENV_APP_TOKEN,
      })
        .then((data) => {
