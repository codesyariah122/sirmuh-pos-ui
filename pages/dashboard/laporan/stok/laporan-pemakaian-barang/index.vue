  ...
  <div class="flex flex-wrap mt-4">
    <div :class="`${$nuxt.showSidebar ? 'w-full mb-12' : '-ml-10 max-w-full'}`">
      <cards-card-table
        ...
        types="laporan-pemakaian-barang"
        queryType="LAPORAN_PEMAKAIAN_BARANG"
        queryMiddle="laporan-pemakaian-barang"
        :headers="headers"
        :columns="items"
        :loading="loading"
        :success="success"
...
export default {
  ...
  methods: {
    handleFilterBarang(param, types) {
      if (types === "laporan-pemakaian-barang") {
    ...
    deleteBarang(id) {
      this.loading = true;
      this.options = "delete-laporan-pemakaian-barang";
      deleteData({
        api_url: `${this.api_url}/laporan-pemakaian-barang/${id}`,
        token: this.token.token,
        api_key: process.env.NUXT_ENV_APP_TOKEN,
      })
        .then((data) => {
