<template>
  <div class="flex flex-wrap mt-4">
    <div class="w-full mb-12 px-4">
      <cards-card-table color="dark" title="DATA LAPORAN PIUTANG" types="data-laporan-piutang"
        queryType="DATA_LAPORAN_PIUTANG" queryMiddle="data-laporan-piutang" :headers="headers" :columns="items"
...
export default {
  ...
  methods: {
    ...
    deleteBarang(id) {
      this.loading = true;
      this.options = "delete-data-laporan-piutang";
      deleteData({
        api_url: `${this.api_url}/data-laporan-piutang/${id}`,
        token: this.token.token,
        api_key: process.env.NUXT_ENV_APP_TOKEN,
      })
        .then((data) => {
