<template>
  <div class="flex flex-wrap mt-4">
    <div class="w-full mb-12 px-4">
      <cards-card-table color="dark" title="GRAFIK PENJUALAN" types="grafik-penjualan" queryType="GRAFIK_PENJUALAN"
        queryMiddle="grafik-penjualan" :headers="headers" :columns="items" :loading="loading" :success="success"
...
export default {
  ...
  methods: {
    ...
    deleteBarang(id) {
      this.loading = true;
      this.options = "delete-grafik-penjualan";
      deleteData({
        api_url: `${this.api_url}/grafik-penjualan/${id}`,
        token: this.token.token,
        api_key: process.env.NUXT_ENV_APP_TOKEN,
      })
        .then((data) => {
